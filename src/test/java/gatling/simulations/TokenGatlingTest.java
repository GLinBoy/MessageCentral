 ...
 */
public class TokenGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tokens").get("/api/tokens").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new token")
                        .post("/api/tokens")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"token\": \"SAMPLE_TEXT\"" +
                                ", \"disable\": null" +
                                ", \"deprecateAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"roles\": 0" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created token").get("${new_token_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created token").delete("${new_token_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Token entity").exec(scn);
