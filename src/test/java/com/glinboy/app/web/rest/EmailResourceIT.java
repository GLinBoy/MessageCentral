...
class EmailResourceIT {
    ...
    public static Email createEntity(EntityManager em) {
        Email email = new Email().receiver(DEFAULT_RECEIVER).subject(DEFAULT_SUBJECT).content(DEFAULT_CONTENT).status(DEFAULT_STATUS);
        return email;
    }
    ...
    public static Email createUpdatedEntity(EntityManager em) {
        Email email = new Email().receiver(UPDATED_RECEIVER).subject(UPDATED_SUBJECT).content(UPDATED_CONTENT).status(UPDATED_STATUS);
        return email;
    }
    ...
    @Test
    @Transactional
    void getAllEmailsByStatusIsNotEqualToSomething() throws Exception {
        // Initialize the database
        emailRepository.saveAndFlush(email);

        // Get all the emailList where status not equals to DEFAULT_STATUS
        defaultEmailShouldNotBeFound("status.notEquals=" + DEFAULT_STATUS);

        // Get all the emailList where status not equals to UPDATED_STATUS
        defaultEmailShouldBeFound("status.notEquals=" + UPDATED_STATUS);
    }
    ...
    void putNewEmail() throws Exception {
        ...
        updatedEmail.receiver(UPDATED_RECEIVER).subject(UPDATED_SUBJECT).content(UPDATED_CONTENT).status(UPDATED_STATUS);
    ...
    void fullUpdateEmailWithPatch() throws Exception {
        ...
        partialUpdatedEmail.receiver(UPDATED_RECEIVER).subject(UPDATED_SUBJECT).content(UPDATED_CONTENT).status(UPDATED_STATUS);
