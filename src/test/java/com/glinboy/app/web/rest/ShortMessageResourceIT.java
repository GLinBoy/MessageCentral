...
class ShortMessageResourceIT {
    ...
    public static ShortMessage createEntity(EntityManager em) {
        ShortMessage shortMessage = new ShortMessage().phoneNumber(DEFAULT_PHONE_NUMBER).content(DEFAULT_CONTENT).status(DEFAULT_STATUS);
        return shortMessage;
    }
    ...
    public static ShortMessage createUpdatedEntity(EntityManager em) {
        ShortMessage shortMessage = new ShortMessage().phoneNumber(UPDATED_PHONE_NUMBER).content(UPDATED_CONTENT).status(UPDATED_STATUS);
        return shortMessage;
    }
    ...
    @Test
    @Transactional
    void getAllShortMessagesByStatusIsNotEqualToSomething() throws Exception {
        // Initialize the database
        shortMessageRepository.saveAndFlush(shortMessage);

        // Get all the shortMessageList where status not equals to DEFAULT_STATUS
        defaultShortMessageShouldNotBeFound("status.notEquals=" + DEFAULT_STATUS);

        // Get all the shortMessageList where status not equals to UPDATED_STATUS
        defaultShortMessageShouldBeFound("status.notEquals=" + UPDATED_STATUS);
    }
    ...
    void putNewShortMessage() throws Exception {
        ...
        updatedShortMessage.phoneNumber(UPDATED_PHONE_NUMBER).content(UPDATED_CONTENT).status(UPDATED_STATUS);
    ...
    void fullUpdateShortMessageWithPatch() throws Exception {
        ...
        partialUpdatedShortMessage.phoneNumber(UPDATED_PHONE_NUMBER).content(UPDATED_CONTENT).status(UPDATED_STATUS);
